...
# We will test multiple feedback scale values: 0.0 (no feedback), 0.02 (4% DTR reduction), 0.04 (8% DTR reduction), 0.05 (10% DTR reduction), 0.10 (20% DTR reduction)
for feedback_factor in [0.0, 0.02, 0.04, 0.05, 0.10]:
    ...
    print(f"Feedback Factor {feedback_factor:.2f}: Simulated Rain = {sim_df['Predicted_Rainfall'].sum():.2f} mm | Wet Days = {(sim_df['IsRainy'] == 1).sum()} / 365")
